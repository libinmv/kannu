...
struct ChatInputView: View {
    ...
    private func sendMessage() {
        ...
        switch provider {
        case .gemini:
            apiKey = SecureSecretsStore.value(for: .geminiAPIKey)
        case .openai:
            apiKey = SecureSecretsStore.value(for: .openaiAPIKey)
        case .claude:
            apiKey = SecureSecretsStore.value(for: .claudeAPIKey)
        case .local:
            // Local models don't need API keys
            apiKey = "local"
        case .groq:
            apiKey = SecureSecretsStore.value(for: .groqAPIKey)
        }
