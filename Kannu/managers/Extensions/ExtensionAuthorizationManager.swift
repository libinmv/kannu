...
final class ExtensionAuthorizationManager: ObservableObject {
    ...
    private let persistenceQueue = DispatchQueue(label: "com.kannu.extensions.authorization", qos: .utility)
    ...
    private func defaultScopes() -> Set<ExtensionPermissionScope> {
        // Least-privilege defaults for new extension entries.
    ...
    private func normalizeState() {
        ...
        entries = entries.map { entry in
            var copy = entry
            // Legacy migration: file-sharing scope now requires explicit user opt-in.
            copy.allowedScopes.remove(.fileSharing)
            return copy
        }
    }
}
