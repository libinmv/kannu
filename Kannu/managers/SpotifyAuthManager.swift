...
final class SpotifyAuthManager: ObservableObject {
    ...
    var configuredCookie: String {
        Self.sanitizeCookie(SecureSecretsStore.value(for: .spotifySPDCCookie))
    }
    ...
    func validateSession() async {
        ...
        SecureSecretsStore.set(sanitizedCookie, for: .spotifySPDCCookie)
        clearCachedToken()
    ...
    func validAccessToken(forceRefresh: Bool = false) async -> String? {
        let sanitizedCookie = configuredCookie
        if sanitizedCookie != SecureSecretsStore.value(for: .spotifySPDCCookie) {
            SecureSecretsStore.set(sanitizedCookie, for: .spotifySPDCCookie)
        }
