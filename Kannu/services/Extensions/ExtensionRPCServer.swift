...
/// Accepts only loopback clients on port 9020 for JSON-RPC 2.0 over WebSocket.
@MainActor
final class ExtensionRPCServer {
    ...
    private let allowedLoopbackHosts: Set<String> = ["127.0.0.1", "::1", "[::1]", "localhost"]
    ...
    private func handleNewConnection(_ nwConnection: NWConnection) {
        guard isLoopback(endpoint: nwConnection.endpoint) else {
            logDiagnostics("Rejected non-loopback RPC connection from \(nwConnection.endpoint)")
            nwConnection.cancel()
            return
        }
    ...
    private func isLoopback(endpoint: NWEndpoint) -> Bool {
        guard case let .hostPort(host, _) = endpoint else { return false }
        let normalized = host.debugDescription.replacingOccurrences(of: "\"", with: "").lowercased()
        return allowedLoopbackHosts.contains(normalized)
    }
