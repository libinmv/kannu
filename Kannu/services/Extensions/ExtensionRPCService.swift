...
final class ExtensionRPCService {
    ...
    private func handleRequestAuthorization(params: RPCParams?, id: String) -> Codable {
        ...
        guard bi == bundleIdentifier else {
            return errorResponse(code: RPCErrorCode.unauthorized, message: "Connection identity mismatch", id: id)
        }
        ...
        // Security hardening: keep new clients pending until explicitly approved by user.
        logDiagnostics("Authorization requested for pending RPC client \(bi)")
    ...
    private func handlePresentLiveActivity(params: RPCParams?, id: String) -> Codable {
        if let err = requireAuthorized(scope: .liveActivities, id: id) { return err }
        ...
        do {
            ...
            guard descriptor.bundleIdentifier == bundleIdentifier else {
                return errorResponse(code: RPCErrorCode.unauthorized, message: "Descriptor identity mismatch", id: id)
            }
    ...
    private func handleDismissLiveActivity(params: RPCParams?, id: String) -> Codable {
        ...
        if let requestedBI = params?["bundleIdentifier"]?.stringValue, requestedBI != bundleIdentifier {
            return errorResponse(code: RPCErrorCode.unauthorized, message: "Connection identity mismatch", id: id)
        }
    ...
    private func handlePresentLockScreenWidget(params: RPCParams?, id: String) -> Codable {
        if let err = requireAuthorized(scope: .lockScreenWidgets, id: id) { return err }
    ...
    private func handlePresentNotchExperience(params: RPCParams?, id: String) -> Codable {
        if let err = requireAuthorized(scope: .notchExperiences, id: id) { return err }
        ...
        if let filesValue = params?["files"],
           case .array(let filesArray) = filesValue {
            for fileValue in filesArray {
                ...
                let standardizedTemp = tempDir.standardizedFileURL.path
                let standardizedFile = fileURL.standardizedFileURL.path
                guard standardizedFile.hasPrefix(standardizedTemp) else { continue }
    ...
    private static func sanitizedFileName(_ raw: String) -> String {
        ...
        let cleaned = candidate.replacingOccurrences(of: "/", with: "_")
        return cleaned.isEmpty ? UUID().uuidString : cleaned
    }

    private func requireAuthorized(scope: ExtensionPermissionScope, id: String) -> RPCErrorResponse? {
        guard Defaults[.enableThirdPartyExtensions] else {
            return errorResponse(code: RPCErrorCode.featureDisabled, message: "Extensions are disabled", id: id)
        }
        switch scope {
        case .liveActivities:
            guard Defaults[.enableExtensionLiveActivities] else {
                return errorResponse(code: RPCErrorCode.featureDisabled, message: "Extension live activities are disabled", id: id)
            }
        case .lockScreenWidgets:
            guard Defaults[.enableExtensionLockScreenWidgets] else {
                return errorResponse(code: RPCErrorCode.featureDisabled, message: "Extension lock screen widgets are disabled", id: id)
            }
        case .notchExperiences:
            guard Defaults[.enableExtensionNotchExperiences] else {
                return errorResponse(code: RPCErrorCode.featureDisabled, message: "Extension notch experiences are disabled", id: id)
            }
        case .fileSharing:
            guard Defaults[.enableExtensionFileSharing] else {
                return errorResponse(code: RPCErrorCode.featureDisabled, message: "Extension file sharing is disabled", id: id)
        ...
        guard let entry = authorizationManager.authorizationEntry(for: bundleIdentifier) else {
            return errorResponse(code: RPCErrorCode.unauthorized, message: "Extension not registered", id: id)
        }
        guard entry.isAuthorized else {
            return errorResponse(code: RPCErrorCode.unauthorized, message: "Extension not authorized", id: id)
        }
        guard entry.allowedScopes.contains(scope) else {
            return errorResponse(code: RPCErrorCode.unauthorized, message: "Scope \(scope.rawValue) not granted", id: id)
        }
        return nil
    }
}
