...
final class AgentStatusNotificationBridge: ObservableObject {
    ...
    private func sendViaPushover(payload: NotificationPayload) async throws {
        let userKey = Defaults[.agentStatusPushoverUserKey].trimmingCharacters(in: .whitespacesAndNewlines)
        let appToken = Defaults[.agentStatusPushoverAppToken].trimmingCharacters(in: .whitespacesAndNewlines)
    ...
    private func sendViaWebhook(payload: NotificationPayload, state: AgentTrafficLightState) async throws {
        let webhook = Defaults[.agentStatusWebhookURL].trimmingCharacters(in: .whitespacesAndNewlines)
